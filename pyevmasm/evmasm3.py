import functools
import binascii
from typing import List, Dict, Tuple, Optional, Iterable, cast, Any
...
class EVMAsm(object):
    ...
    class Instruction(object):
        def __init__(self, opcode: int, name: str, operand_size: int, pops: int, pushes: int, fee: int, description: str, operand: Optional[int] = None, offset: Optional[int] = 0) -> None:
            '''
        ...
        def __eq__(self, other: Any) -> bool:
            ...
            rv: bool = self._opcode == other._opcode and\
        ...
        def __hash__(self) -> int:
            return hash((self._opcode, self._pops, self._pushes, self._offset))

        def __repr__(self) -> str:
            output: str = 'Instruction(0x%x, %r, %d, %d, %d, %d, %r, %r, %r)' % (self._opcode, self._name, self._operand_size,
                                                                            self._pops, self._pushes, self._fee, self._description, self._operand, self._offset)
            return output

        def __str__(self) -> str:
            if self.has_operand:
                output = f"{self.name} #{self.operand:x}"
            else:
                output = f"{self.name}"
            return output
        ...
        def parse_operand(self, buf: Iterable) -> None:
    ...
    def _get_reverse_table() -> Dict[str, Tuple[int, str, int, int, int, int, str]]:
    ...
    @staticmethod
    def assemble_one(assembler: str, offset:int = 0) -> Instruction:
        ...
        '''
        try:
            _reverse_table: Dict[str, Tuple[int, str, int, int, int, int, str]] = EVMAsm._get_reverse_table()
            assembler_: List[str] = assembler.strip().split(' ')
            opcode, name, operand_size, pops, pushes, gas, description = _reverse_table[assembler_[0].upper()]
            if operand_size > 0:
                assert len(assembler_) == 2
                operand : Optional[int] = int(assembler_[1], 0)
            else:
                assert len(assembler_) == 1
                operand = None
    ...
    @staticmethod
    def assemble_all(assembler: str, offset: int = 0) -> Iterable[Instruction]:
        ...
        if isinstance(assembler, str):
            assembler_ : List[str] = assembler.split('\n')
        if assembler_:
            assembler_ = iter(assembler_)
        else:
            assembler_ = iter(assembler)
        for line in assembler_:
            if not line.strip():
                continue
    ...
    @staticmethod
    def disassemble_one(bytecode: Iterable, offset: int = 0) -> Instruction:
    ...
    @staticmethod
    def disassemble_all(bytecode: Iterable, offset: int = 0) -> Iterable[Instruction]:
        ...
        while offset < len(bytecode):
            try:
                insn = EVMAsm.disassemble_one(bytecode[offset:], offset)
            except Exception as e:
                print("Error disassembling: " + repr(e))
                break
            offset += insn.size
            yield insn

    @staticmethod
    def disassemble(bytecode: Iterable, offset : int = 0) -> str:
    ...
    @staticmethod
    def assemble(asmcode: str, offset : int = 0) -> str:
        ...
        '''
        return b''.join(map(lambda x: x.bytes, EVMAsm.assemble_all(asmcode, offset=offset)))

    @staticmethod
    def disassemble_hex(bytecode: str, offset : int = 0) -> str:
        ''' Disassemble an EVM bytecode
        ...
        '''
        if bytecode.startswith(u'0x'):
            bytecode = bytecode[2:]
        bytecode_ : bytes = binascii.unhexlify(bytecode)
        return EVMAsm.disassemble(bytecode_, offset=offset)

    @staticmethod
    def assemble_hex(asmcode : str, offset : int = 0) -> str:
        ''' Assemble an EVM program
        ...
        '''
        hexencoded = EVMAsm.assemble(asmcode, offset=offset)
        return '0x' + binascii.b2a_hex(hexencoded).decode('utf-8')
