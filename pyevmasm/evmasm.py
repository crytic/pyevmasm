...
class InstructionTable(dict):
    ...
    def _search_by_opcode(self, k):
        did_not_found = object()
        value = did_not_found
        try:
            value = Instruction(k, *super(InstructionTable, self).__getitem__(k))
        except KeyError as e:
            if self._previous_fork is not None:
                value = self._previous_fork.get(k, did_not_found)

        if value is did_not_found:
            raise KeyError(k)
    ...
    def __contains__(self, k):
        return super(InstructionTable, self).__contains__(k)
    ...
    def fromkeys(cls, keys, v=None):
        return super(InstructionTable, cls).fromkeys(keys, v)
...
tangerine_whistle_instruction_table = {
    ...
    0xf4 : ('DELEGATECALL', 0, 6, 1, 700, "Message-call into this account with an alternative account's code, but persisting into this account with an alternative account's code."),
    0xff : ('SELFDESTRUCT', 0, 1, 0, 5000, 'Halt execution and register account for later deletion.'),
}
tangerine_whistle_instruction_table = InstructionTable(tangerine_whistle_instruction_table, previous_fork=homestead_instruction_table )

spurious_dragon_instruction_table = {}
spurious_dragon_instruction_table = InstructionTable(spurious_dragon_instruction_table, previous_fork=tangerine_whistle_instruction_table )
...
byzantium_instruction_table = InstructionTable(byzantium_instruction_table, previous_fork=spurious_dragon_instruction_table )
...
constantinople_instruction_table = {
    ...
    0xf5 : ('CREATE2', 0, 3, 1, 32000, 'Behaves identically to CREATE, except using keccak256( 0xff ++ address ++ salt ++ keccak256(init_code)))[12:] as the address where the contract is initialized at')
}
constantinople_instruction_table = InstructionTable(constantinople_instruction_table, previous_fork=byzantium_instruction_table )
